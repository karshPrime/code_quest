...
SPAWN_CYCLE = {
    "Early_game": ["W", "W", "F", "W", "W"],
    "Attacked": ["F", "F", "W", "F", "F"],
    "Far_hill": ["F", "W", "S", "W", "F"],
    "Close_hill": ["S", "F", "S", "W", "F"],
    "Rush": ["F", "F", "F", "W", "F"],
    "Econ_And_Harass": ["W", "F", "W", "W", "F"],
    "Snipe": ["F", "F", "W", "F", "W"]
}
...
def handle_events(events):
    ...
    ant_to_spawn = {
        "W": int(STRATEGY[curr_strat][0] * stats.general.MAX_SPAWNS_PER_TICK/100),
        "F": int(STRATEGY[curr_strat][1] * stats.general.MAX_SPAWNS_PER_TICK/100),
        "S": int(STRATEGY[curr_strat][2] * stats.general.MAX_SPAWNS_PER_TICK/100),
    }
    # Can I spawn ants?
    ant_spawned = 0
    ...
    while (
        ...
        ant_spawned < stats.general.MAX_SPAWNS_PER_TICK and
        i < len(new_fighters)
    ):
    ...
    while (
        total_ants < stats.general.MAX_ANTS_PER_PLAYER and
        ant_spawned < stats.general.MAX_SPAWNS_PER_TICK
    ):
        cost = 0
        ant_spawn = SPAWN_CYCLE[curr_strat][ant_spawned%stats.general.MAX_SPAWNS_PER_TICK]
        if  ant_spawn == "W":
            cost = stats.ants.Worker.COST
        elif ant_spawn == "F":
            cost = stats.ants.Fighter.COST
        else:
            cost = stats.ants.Settler.COST

        if (
            my_energy >= cost and
            ant_to_spawn[ant_spawn] > 0
        ):
            if(ant_spawn == "F"):
